...
class DogsApiGatewayTest extends TestCase
{
    private DogsApiGateway $sut;
    ...
    protected function setUp(): void
    {
        ...
        $this->sut = new DogsApiGateway($this->clientGetRequestSender);
    ...
    /**
     * @covers \PetAdoption\apis\gateways\DogsApiGateway::__construct
     */
    public function testShouldSetUpTheRightApiCredentialsOnClassConstruct()
    {
        $reflectionClass = new ReflectionClass(DogsApiGateway::class);
        ...
        $this->assertEquals($urlProperty->getValue($this->sut), "https://api.thedogapi.com/v1/images/search");
        $this->assertEquals($headersProperty->getValue($this->sut), ['x-api-key' => getenv()['DOGS_API_TOKEN']]);
    ...
    /**
     * @covers \PetAdoption\apis\gateways\DogsApiGateway::request
     */
    public function testShouldCallClientGetRequestSenderWithCorrectValues()
    {
        $apiLink = "https://api.thedogapi.com/v1/images/search?limit=80&order=Asc&page=";
        $headers = ['x-api-key' => getenv()['DOGS_API_TOKEN']];
        $apiReturn = [
            (object)[
